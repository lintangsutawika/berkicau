...
modelFolder = re.split('\n',open("./Models/models.txt", "rb").read())
models = []
...
for ind,mod in enumerate(models):
    ...
    for nFold, (train_index, test_index) in enumerate(kf.split(data)):
        if nFold == 0:
            mod, _ = entityTagger.trainModel(nFold, train_index, test_index, (word2vec), trainedModel=mod ,model_name="model_Retrained_{}.pth".format(ind), num_epochs=50)
    newModels.append(mod)
...
for index,testTrain in enumerate(tokenizedTest):
    ...
    out = []
    for num,model in enumerate(newModels):
...
with open("output_TweetData_CombinedFalse.txt", 'w') as txtFile:
    for lines in text:
        txtFile.write(lines+"\r\n")
